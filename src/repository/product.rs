...
impl repository::Repository{
    pub fn all_products(&mut self) -> Vec<domain::Product> {
        const SELECT_ALL: &str = "SELECT myid, name FROM products ORDER BY name";
    ...
    pub fn products_start_withid(&mut self, value: &str) -> Vec<domain::Product>{
        const SELECT_NAME_START:&str = 
            "SELECT myid, name FROM products WHERE name ilike $1::varchar ORDER BY name";
        let mut products: Vec<domain::Product> = Vec::new();
        let value = format!("{}%",value);
        let result = self.db.query(SELECT_NAME_START, &[&value]).unwrap();
        for row in result{
            let product = domain::Product{
                id: row.get(0),
                name: row.get(1),
